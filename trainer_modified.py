...
from utils import rand_bbox, generate_attentive_mask_threshold, generate_attentive_mask, print_v
...
def train_MACM(model, train_loader, optimizer, scheduler, criterion, cur_epoch, device, **kwargs):
    ...
    for idx, data in enumerate(train_loader):
        ...
        if idx%100 == 0 and cur_epoch % 20 == 0:
            ...
            fig, ax = plt.subplots(1,1,figsize=(8,(batch.size(0)//8)+1))
            ax.imshow(input_ex)
            ax.set_title(f"Train MACM Batch Examples\nCut_Prob:{cut_prob}, Cur_Target: {target_stage_name}, Num_occlusion: {n_occluded_pixels} ")
            ax.axis('off')
            fig.savefig(os.path.join(save_path, f"Train_BatchSample_E{cur_epoch}_I{idx}.png"))
            plt.draw()
            plt.clf()
    ...
    for idx, data in enumerate(train_loader):
        ...
        if r < cut_prob:
            ...
            if mask_mode == 'top':
                ...
                n_occluded_pixels = attention_masks[attention_masks == 1].sum()
            else:
                assert f'Invalid mask_mode: {mask_mode}'
