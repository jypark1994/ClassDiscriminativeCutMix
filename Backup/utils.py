...
def generate_attentive_mask_threshold(attention_map, threshold):
    """
        Author: Junyoung Park (jy_park@inu.ac.kr)
        Input:
            attention_map   (Tensor) : NxWxH tensor after GAP.
            threshold       (float) : Threshold for the intensity to apply mask. 
        Output:
            mask            (Tensor) : NxWxH tensor for masking attentive regions
    """
    if threshold < 0 or threshold > 1:
        assert "Threshold should be a float value in 0.0 to 1.0"
