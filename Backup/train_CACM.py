...
def main():
    ...
    if args.dataset.startswith('cifar'):
        ...
        train_dataset = CUB200("~/datasets", transform = train_transforms, train=True, download=False)
        train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=8)
        val_dataset = CUB200("~/datasets", transform = val_transforms, train=False, download=False)
        val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=args.batch_size, shuffle=True, num_workers=8)
